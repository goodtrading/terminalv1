...
import {
  isAlertsDesktopNativeEnabled,
  isAlertsEnabled,
  isAlertsFlowEnabled,
  isAlertsMobilePushEnabled,
  isAlertsWebNotificationsEnabled,
  isHeatmapEnabled,
} from "./lib/runtimeEnv";
...
export async function registerRoutes(
  ...
  app.get("/api/runtime/features", (_req: Request, res: Response) => {
    res.json({
      ...
      alertsDesktopNativeEnabled: isAlertsDesktopNativeEnabled(),
  ...
  if (process.env.DATABASE_URL) {
    try {
      ...
      const { registerAlertRoutes } = await import("./routes/alerts.routes");
  ...
  app.get("/api/options/deribit/book", async (req, res) => {
    ...
    try {
      const currency = ((req.query.currency as string) || "BTC").toUpperCase();
      ...
      const cacheKey = `deribit-book:${currency}:${expiry ?? "auto"}`;
      const payload = await cachedFetch(
        cacheKey,
        {
          ttlMs: 15_000,
        ...
        () => fetchDeribitOptionsBook({ currency, expiry }),
      );
    ...
    for (const currency of ["BTC", "ETH"] as const) {
      try {
        await cachedFetch(
          ...
          () => fetchDeribitOptionsBook({ currency }),
        );
        if (process.env.NODE_ENV !== "production") {
          console.debug("[OPTIONS_WARM_CACHE]", { currency });
        }
      } catch (error) {
        console.warn("[OPTIONS_WARM_CACHE_FAILED]", currency, error);
      }
    }
  })();
