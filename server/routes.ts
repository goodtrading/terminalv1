...
export async function registerRoutes(
  ...
  // Historical data helper (optional fallback if needed, but per instructions we only use WS now)
  app.get("/api/chart/candles", async (req, res) => {
    return res.status(410).json({ error: "DEPRECATED", details: "Terminal moved to WebSocket streaming." });
  });
