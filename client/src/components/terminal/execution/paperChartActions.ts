...
export async function postPaperClosePartial(percent: number): Promise<{
  ...
  const res = await fetch("/api/paper/position/close-partial", {
    method: "POST",
...
export async function postPaperCancelOrder(orderId: string): Promise<void> {
  const res = await fetch(`/api/paper/orders/${encodeURIComponent(orderId)}/cancel`, {
    method: "POST",
  });
...
export async function patchPaperOrderPrice(
  ...
  const res = await fetch(`/api/paper/orders/${encodeURIComponent(orderId)}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
