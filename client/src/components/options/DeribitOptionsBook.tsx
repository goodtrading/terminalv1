...
// View mode types
type OptionsViewMode = "SIMPLE" | "PRO" | "GT";
...
export default function DeribitOptionsBook() {
  ...
  const [viewMode, setViewMode] = useState<OptionsViewMode>("GT");
  ...
  // Render helpers for different view modes
  const renderCallCells = (row: OptionsDerivedRow) => {
    const badgeContext = {
      institutionalData,
      bookData,
      optionsIntel: {
    ...
    const badges = getRowBadges(row, badgeContext).call;
    
    switch (viewMode) {
      case "SIMPLE":
        return (
          <>
            <td className="text-right px-3 py-2 text-xs font-mono border-r border-terminal-border/20">
            ...
            <td className="text-right px-3 py-2 text-xs font-mono text-green-400 border-r border-terminal-border/20">
              <div className="text-xs">
                {formatBidAsk(row.call?.bidPrice, row.call?.askPrice)}
              </div>
            </td>
          </>
        );
      ...
      case "PRO":
        return (
          <>
            <td className="text-right px-3 py-1.5 text-xs font-mono border-r border-terminal-border/20">
              <div className="flex flex-col gap-1 items-end">
            ...
            {columnVisibility.hasDelta && (
              <td className="text-right px-3 py-1.5 text-xs font-mono text-green-400/60 border-r border-terminal-border/20">
                {formatNumber(row.call?.delta, 3)}
              </td>
            )}
            {columnVisibility.hasIv && (
              <td className="text-right px-3 py-1.5 text-xs font-mono text-green-400/60 border-r border-terminal-border/20">
                {formatPercent(row.call?.bidIv)}
              </td>
            )}
            <td className="text-right px-3 py-1.5 text-xs font-mono text-green-400 font-medium border-r border-terminal-border/20">
              {formatPrice(row.call?.bidPrice)}
            </td>
            <td className="text-right px-3 py-1.5 text-xs font-mono text-green-400 font-medium border-r border-terminal-border/20">
              {formatPrice(row.call?.askPrice)}
            </td>
            {columnVisibility.hasIv && (
              <td className="text-right px-3 py-1.5 text-xs font-mono text-green-400/60 border-r border-terminal-border/20">
                {formatPercent(row.call?.askIv)}
              </td>
            )}
            {columnVisibility.hasSize && (
              <td className="text-right px-3 py-1.5 text-xs font-mono text-green-400/70 border-r border-terminal-border/20">
                {formatNumber(row.call?.bidSize)}
              </td>
            )}
          </>
        );
      ...
      case "GT":
        return (
          <>
            ...
            <td className="text-right px-3 py-1.5 text-xs font-mono text-green-200 border-r border-terminal-border/20">
              {formatNotional(row.derived.callNotionalUsd)}
            </td>
            <td className="text-right px-3 py-1.5 text-xs font-mono text-green-400 border-r border-terminal-border/20">
              <div className="text-xs">
                {formatBidAsk(row.call?.bidPrice, row.call?.askPrice)}
              </div>
            </td>
  ...
  const renderCenterCells = (row: OptionsDerivedRow) => {
    const badgeContext = {
      institutionalData,
      bookData,
      optionsIntel: {
    ...
    const badges = getRowBadges(row, badgeContext).center;
    
    switch (viewMode) {
      case "SIMPLE":
        return (
          <td className={cn(
            "px-2 py-2 text-center border-r border-terminal-border/20 relative",
            isAtmStrike(row.strike) && "bg-blue-500/10 border-l-2 border-r-2 border-blue-400/40"
          )}>
            <div className="flex flex-col gap-1 items-center">
              <div className={cn(
                "text-base font-bold font-mono",
                isAtmStrike(row.strike) ? "text-blue-400" : "text-white"
              )}>
                {formatPrice(row.strike)}
              </div>
              <div className="text-xs text-blue-200">
                {formatDistance(row.derived.distanceToSpotPct)}
              </div>
      ...
      case "PRO":
        return (
          <td className={cn(
            "w-24 px-2 py-1.5 text-center border-r border-terminal-border/20 relative",
            isAtmStrike(row.strike) && "bg-blue-500/10 border-l-2 border-r-2 border-blue-400/40"
          )}>
            <div className="flex flex-col gap-1 items-center">
              <div className={cn(
                "text-sm font-bold font-mono",
                isAtmStrike(row.strike) ? "text-blue-400" : "text-white/90"
              )}>
                {formatPrice(row.strike)}
              </div>
              <div className="flex flex-wrap gap-1 justify-center">
      ...
      case "GT":
        return (
          <>
            ...
            <td className="px-2 py-1.5 text-center border-r border-terminal-border/20">
              <div className="text-xs text-blue-200">
                {formatDistance(row.derived.distanceToSpotPct)}
              </div>
            </td>
            <td className="px-2 py-1.5 text-center border-r border-terminal-border/20">
              {(() => {
                ...
                const mainSignal = getMainSignal(row, signalContext);
                return mainSignal ? (
                  <BadgePill badge={mainSignal} />
                ) : (
  ...
  const renderPutCells = (row: OptionsDerivedRow) => {
    const badgeContext = {
      institutionalData,
      bookData,
      optionsIntel: {
    ...
    const badges = getRowBadges(row, badgeContext).put;
    
    switch (viewMode) {
      case "SIMPLE":
        return (
          <>
            <td className="text-left px-3 py-2 text-xs font-mono text-red-400 border-r border-terminal-border/20">
              <div className="text-xs">
                {formatBidAsk(row.put?.bidPrice, row.put?.askPrice)}
              </div>
            </td>
            <td className="text-left px-3 py-2 text-xs font-mono border-r border-terminal-border/20">
      ...
      case "PRO":
        return (
          <>
            {columnVisibility.hasSize && (
              <td className="text-left px-3 py-1.5 text-xs font-mono text-red-400/70 border-r border-terminal-border/20">
                {formatNumber(row.put?.askSize)}
              </td>
            )}
            {columnVisibility.hasIv && (
              <td className="text-left px-3 py-1.5 text-xs font-mono text-red-400/60 border-r border-terminal-border/20">
                {formatPercent(row.put?.bidIv)}
              </td>
            )}
            <td className="text-left px-3 py-1.5 text-xs font-mono text-red-400 font-medium border-r border-terminal-border/20">
              {formatPrice(row.put?.bidPrice)}
            </td>
            <td className="text-left px-3 py-1.5 text-xs font-mono text-red-400 font-medium border-r border-terminal-border/20">
              {formatPrice(row.put?.askPrice)}
            </td>
            {columnVisibility.hasIv && (
              <td className="text-left px-3 py-1.5 text-xs font-mono text-red-400/60 border-r border-terminal-border/20">
                {formatPercent(row.put?.askIv)}
              </td>
            )}
            {columnVisibility.hasDelta && (
              <td className="text-left px-3 py-1.5 text-xs font-mono text-red-400/60 border-r border-terminal-border/20">
            ...
            <td className="text-left px-3 py-1.5 text-xs font-mono">
              <div className="flex flex-col gap-1 items-start">
      ...
      case "GT":
        return (
          <>
            <td className="text-left px-3 py-1.5 text-xs font-mono text-red-400 border-r border-terminal-border/20">
              <div className="text-xs">
                {formatBidAsk(row.put?.bidPrice, row.put?.askPrice)}
              </div>
            </td>
            <td className="text-left px-3 py-1.5 text-xs font-mono text-red-200 border-r border-terminal-border/20">
              {formatNotional(row.derived.putNotionalUsd)}
            </td>
            <td className="text-left px-3 py-1.5 text-xs font-mono">
  ...
  // Table headers helper for different view modes
        ...
        return (
          <>
            ...
            <tr className="sticky top-0 bg-terminal-bg z-30">
              <th colSpan={2} className="bg-green-500/10 px-3 py-2 h-8 text-center border-r border-terminal-border/50">
                <div className="text-xs font-bold text-green-400 uppercase tracking-wider">Calls</div>
              </th>
              <th className="w-40 bg-gray-500/10 px-2 py-2 h-8 border-r border-terminal-border/50">
                <div className="text-xs font-bold text-gray-300 uppercase tracking-wider">Strike</div>
              </th>
              <th colSpan={2} className="bg-red-500/10 px-3 py-2 h-8 text-center">
                <div className="text-xs font-bold text-red-400 uppercase tracking-wider">Puts</div>
              </th>
            </tr>
            {/* Column Headers Row */}
            <tr className="sticky top-8 bg-terminal-bg z-29">
              {/* Calls Headers */}
              <th className="text-right px-3 py-2 h-6 text-[10px] font-medium text-terminal-muted border-r border-terminal-border/50">OI</th>
              <th className="text-right px-3 py-2 h-6 text-[10px] font-medium text-terminal-muted border-r border-terminal-border/50">Bid/Ask</th>
              
              {/* Strike Headers */}
              <th className="px-2 py-2 h-6 text-center text-[10px] font-medium text-terminal-muted border-r border-terminal-border/50">
                Strike
              </th>
              <th className="px-2 py-2 h-6 text-center text-[10px] font-medium text-terminal-muted border-r border-terminal-border/50">
                Dist %
              ...
              {/* Puts Headers */}
              <th className="text-left px-3 py-2 h-6 text-[10px] font-medium text-terminal-muted">Bid/Ask</th>
              <th className="text-left px-3 py-2 h-6 text-[10px] font-medium text-terminal-muted">OI</th>
            </tr>
          </>
        );
      ...
      case "PRO":
        return (
          <>
            {/* Group Headers Row */}
            <tr className="sticky top-0 bg-terminal-bg z-30">
              <th colSpan={columnVisibility.hasDelta ? 6 : columnVisibility.hasIv ? 5 : 3} className="bg-green-500/10 px-3 py-2 h-8 text-center border-r border-terminal-border/50">
                <div className="text-xs font-bold text-green-400 uppercase tracking-wider">Calls / Compras</div>
              </th>
              <th className="w-24 bg-gray-500/10 px-2 py-2 h-8 border-r border-terminal-border/50">
                <div className="text-xs font-bold text-gray-300 uppercase tracking-wider">Strike</div>
              </th>
              <th colSpan={columnVisibility.hasDelta ? 6 : columnVisibility.hasIv ? 5 : 3} className="bg-red-500/10 px-3 py-2 h-8 text-center">
                <div className="text-xs font-bold text-red-400 uppercase tracking-wider">Puts / Ventas</div>
              </th>
            </tr>
            {/* Column Headers Row */}
            <tr className="sticky top-8 bg-terminal-bg z-29">
              ...
              <th className="text-right px-3 py-2 h-6 text-[10px] font-medium text-terminal-muted border-r border-terminal-border/50">OI</th>
              {columnVisibility.hasDelta && (
                <th className="text-right px-3 py-2 h-6 text-[10px] font-medium text-terminal-muted border-r border-terminal-border/50">Δ</th>
              )}
              {columnVisibility.hasIv && (
                <th className="text-right px-3 py-2 h-6 text-[10px] font-medium text-terminal-muted border-r border-terminal-border/50">IV Bid</th>
              )}
              <th className="text-right px-3 py-2 h-6 text-[10px] font-medium text-terminal-muted border-r border-terminal-border/50">Bid</th>
              <th className="text-right px-3 py-2 h-6 text-[10px] font-medium text-terminal-muted border-r border-terminal-border/50">Ask</th>
              {columnVisibility.hasIv && (
                <th className="text-right px-3 py-2 h-6 text-[10px] font-medium text-terminal-muted border-r border-terminal-border/50">IV Ask</th>
              )}
              {columnVisibility.hasSize && (
                <th className="text-right px-3 py-2 h-6 text-[10px] font-medium text-terminal-muted border-r border-terminal-border/50">Size</th>
              )}
              
              {/* Strike Header */}
              <th className="w-24 px-2 py-2 h-6 text-center text-[10px] font-medium text-terminal-muted border-r border-terminal-border/50">
                Ejecución
              </th>
              
              {/* Puts Headers */}
              {columnVisibility.hasSize && (
                <th className="text-left px-3 py-2 h-6 text-[10px] font-medium text-terminal-muted">Size</th>
              )}
              {columnVisibility.hasIv && (
                <th className="text-left px-3 py-2 h-6 text-[10px] font-medium text-terminal-muted">IV Bid</th>
              )}
              <th className="text-left px-3 py-2 h-6 text-[10px] font-medium text-terminal-muted">Bid</th>
              <th className="text-left px-3 py-2 h-6 text-[10px] font-medium text-terminal-muted">Ask</th>
              {columnVisibility.hasIv && (
                <th className="text-left px-3 py-2 h-6 text-[10px] font-medium text-terminal-muted">IV Ask</th>
              )}
              {columnVisibility.hasDelta && (
                <th className="text-left px-3 py-2 h-6 text-[10px] font-medium text-terminal-muted">Δ</th>
              )}
              <th className="text-left px-3 py-2 h-6 text-[10px] font-medium text-terminal-muted">OI</th>
            </tr>
          </>
      ...
      case "GT":
        return (
          <>
            {/* Group Headers Row */}
            <tr className="sticky top-0 bg-terminal-bg z-30">
              <th colSpan={3} className="bg-green-500/10 px-3 py-2 h-8 text-center border-r border-terminal-border/50">
                <div className="text-xs font-bold text-green-400 uppercase tracking-wider">Calls / Compras</div>
              </th>
              <th className="w-48 bg-gray-500/10 px-2 py-2 h-8 border-r border-terminal-border/50">
                <div className="text-xs font-bold text-gray-300 uppercase tracking-wider">Strike</div>
              </th>
              <th colSpan={3} className="bg-red-500/10 px-3 py-2 h-8 text-center">
                <div className="text-xs font-bold text-red-400 uppercase tracking-wider">Puts / Ventas</div>
              </th>
            </tr>
            {/* Column Headers Row */}
            <tr className="sticky top-8 bg-terminal-bg z-29">
              {/* Calls Headers */}
              <th className="text-right px-3 py-2 h-6 text-[10px] font-medium text-terminal-muted border-r border-terminal-border/50">OI</th>
              <th className="text-right px-3 py-2 h-6 text-[10px] font-medium text-terminal-muted border-r border-terminal-border/50">Notional</th>
              <th className="text-right px-3 py-2 h-6 text-[10px] font-medium text-terminal-muted border-r border-terminal-border/50">Bid/Ask</th>
              ...
              {/* Puts Headers */}
              <th className="text-left px-3 py-2 h-6 text-[10px] font-medium text-terminal-muted">Bid/Ask</th>
              <th className="text-left px-3 py-2 h-6 text-[10px] font-medium text-terminal-muted">Notional</th>
              <th className="text-left px-3 py-2 h-6 text-[10px] font-medium text-terminal-muted">OI</th>
            </tr>
          </>
        );
  ...
  const currentColumns = useMemo(() => {
    switch (viewMode) {
      case "SIMPLE": return getSimpleColumns();
      case "PRO": return getProColumns();
      case "GT": return getGtColumns();
      default: return getSimpleColumns();
    }
  ...
  return (
    <div className="h-full flex flex-col bg-terminal-bg text-terminal-text">
      ...
      <div className="flex flex-wrap items-center gap-2 p-3 border-b border-terminal-border shrink-0">
        ...
        {/* View Mode Selector */}
        <div className="flex gap-1 border-l border-terminal-border/30 pl-2">
          {(["SIMPLE", "PRO", "GT"] as const).map(mode => (
            <button
              key={mode}
